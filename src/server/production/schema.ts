/**
 * Idempotent PostgreSQL schema for the production Machine Console.
 *
 * IDs and opaque secrets are generated by the application. The database only
 * receives hashes for wallet sessions and machine credentials.
 */
...
CREATE TABLE IF NOT EXISTS mc_wallets (
  ...
  user_id TEXT NOT NULL REFERENCES mc_users(id) ON DELETE CASCADE,
  verified_at TIMESTAMPTZ NOT NULL,
  UNIQUE (user_id, address)
);

CREATE TABLE IF NOT EXISTS mc_auth_challenges (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS mc_auth_sessions (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES mc_users(id) ON DELETE CASCADE,
  wallet_address TEXT NOT NULL REFERENCES mc_wallets(address) ON DELETE CASCADE,
  token_hash TEXT NOT NULL UNIQUE,
  csrf_hash TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  expires_at TIMESTAMPTZ NOT NULL,
  revoked_at TIMESTAMPTZ,
  CONSTRAINT mc_auth_sessions_verified_wallet_fk
    FOREIGN KEY (user_id, wallet_address) REFERENCES mc_wallets(user_id, address) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS mc_machines (
  machine_id TEXT PRIMARY KEY,
  owner_user_id TEXT NOT NULL REFERENCES mc_users(id) ON DELETE CASCADE,
  label TEXT NOT NULL,
  role TEXT NOT NULL CHECK (role IN ('robot_arm','drone','sensor','rover','warehouse_bot','edge_node')),
  wallet_address TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  UNIQUE (machine_id, owner_user_id),
  FOREIGN KEY (owner_user_id, wallet_address) REFERENCES mc_wallets(user_id, address) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS mc_machine_capabilities (
  ...
  capability TEXT NOT NULL CHECK (capability IN ('inspection','delivery','pick_place','mapping','inference','sensing','compute','charging','audit_capture')),
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (machine_id, capability),
  FOREIGN KEY (machine_id, owner_user_id) REFERENCES mc_machines(machine_id, owner_user_id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS mc_machine_capabilities_owner ON mc_machine_capabilities(owner_user_id, capability);

CREATE TABLE IF NOT EXISTS mc_runtime_sessions (
  ...
  mode TEXT NOT NULL CHECK (mode IN ('fixture','live-read')),
  nonce_hash TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS mc_work_orders (
  work_order_id TEXT PRIMARY KEY,
  owner_user_id TEXT NOT NULL REFERENCES mc_users(id) ON DELETE CASCADE,
  machine_id TEXT,
  stage TEXT NOT NULL CHECK (stage IN ('queued','assigned','preparing','working','proof_submitted','settled','failed','cancelled')),
  ...
  settlement_chain TEXT NOT NULL CHECK (settlement_chain='solana'),
  ...
  created_at TIMESTAMPTZ NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  FOREIGN KEY (machine_id, owner_user_id) REFERENCES mc_machines(machine_id, owner_user_id) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS mc_machine_credentials (
  id TEXT PRIMARY KEY,
  machine_id TEXT NOT NULL REFERENCES mc_machines(machine_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS mc_telemetry_events (
  id TEXT PRIMARY KEY,
  machine_id TEXT NOT NULL REFERENCES mc_machines(machine_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS mc_provider_capabilities (
  id TEXT PRIMARY KEY,
  provider_machine_id TEXT NOT NULL,
  owner_user_id TEXT NOT NULL REFERENCES mc_users(id) ON DELETE CASCADE,
  resource_type TEXT NOT NULL CHECK (resource_type IN ('weather-data','soil-sensor-data','route-map','charging-slot','compute-burst','bandwidth-grant','telemetry-feed')),
  label TEXT NOT NULL,
  unit TEXT NOT NULL,
  rail_tags JSONB NOT NULL,
  availability TEXT NOT NULL CHECK (availability IN ('available','limited','unavailable')),
  price_amount NUMERIC,
  price_asset TEXT,
  created_at TIMESTAMPTZ NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  UNIQUE (provider_machine_id, resource_type, label),
  UNIQUE (id, provider_machine_id),
  FOREIGN KEY (provider_machine_id, owner_user_id) REFERENCES mc_machines(machine_id, owner_user_id) ON DELETE CASCADE,
  CHECK ((price_amount IS NULL AND price_asset IS NULL) OR (price_amount > 0 AND price_asset='SOL'))
);
...
CREATE TABLE IF NOT EXISTS mc_resource_requests (
  id TEXT PRIMARY KEY,
  owner_user_id TEXT NOT NULL REFERENCES mc_users(id) ON DELETE CASCADE,
  requester_machine_id TEXT NOT NULL,
  capability_id TEXT REFERENCES mc_provider_capabilities(id),
  provider_machine_id TEXT REFERENCES mc_machines(machine_id),
  resource_type TEXT NOT NULL CHECK (resource_type IN ('weather-data','soil-sensor-data','route-map','charging-slot','compute-burst','bandwidth-grant','telemetry-feed')),
  quantity NUMERIC NOT NULL CHECK (quantity > 0),
  max_price NUMERIC NOT NULL CHECK (max_price > 0),
  preferred_rails JSONB NOT NULL,
  purpose TEXT NOT NULL,
  quote_amount NUMERIC,
  quote_asset TEXT,
  state TEXT NOT NULL CHECK (state IN ('pending','quoted','accepted','granted','fulfilled','rejected','cancelled')),
  created_at TIMESTAMPTZ NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  UNIQUE (id, owner_user_id),
  FOREIGN KEY (requester_machine_id, owner_user_id) REFERENCES mc_machines(machine_id, owner_user_id) ON DELETE CASCADE,
  CHECK ((quote_amount IS NULL AND quote_asset IS NULL) OR (quote_amount > 0 AND quote_asset='SOL')),
  CHECK (state NOT IN ('accepted','granted','fulfilled') OR
    (capability_id IS NOT NULL AND provider_machine_id IS NOT NULL AND quote_amount IS NOT NULL AND quote_asset IS NOT NULL))
);
...
CREATE TABLE IF NOT EXISTS mc_settlements (
  id TEXT PRIMARY KEY,
  resource_request_id TEXT NOT NULL UNIQUE REFERENCES mc_resource_requests(id) ON DELETE RESTRICT,
  resource_quote_id TEXT NOT NULL,
  owner_user_id TEXT NOT NULL REFERENCES mc_users(id) ON DELETE CASCADE,
  machine_id TEXT NOT NULL REFERENCES mc_machines(machine_id) ON DELETE RESTRICT,
  source_wallet TEXT NOT NULL,
  recipient_wallet TEXT NOT NULL,
  amount_lamports NUMERIC(30,0) NOT NULL CHECK (amount_lamports > 0),
  state TEXT NOT NULL CHECK (state IN ('created','awaiting_signature','submitting','submitted','confirmed','failed','cancelled')),
  unsigned_transaction TEXT,
  transaction_signature TEXT,
  last_valid_block_height NUMERIC(30,0),
  error_code TEXT,
  created_at TIMESTAMPTZ NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  FOREIGN KEY (resource_quote_id, resource_request_id) REFERENCES mc_resource_quotes(id, resource_request_id) ON DELETE RESTRICT,
  FOREIGN KEY (machine_id, owner_user_id) REFERENCES mc_machines(machine_id, owner_user_id) ON DELETE RESTRICT,
  FOREIGN KEY (owner_user_id, source_wallet) REFERENCES mc_wallets(user_id, address) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS mc_resource_receipts (
  ...
  FOREIGN KEY (settlement_id, resource_request_id) REFERENCES mc_settlements(id, resource_request_id) ON DELETE RESTRICT
);
...
-- Safe upgrades for installations that ran an earlier version of this
-- idempotent schema. NOT VALID preserves existing rows while enforcing every
-- newly written row; operators can VALIDATE after auditing legacy data.
...
DO $$ BEGIN
  ALTER TABLE mc_resource_requests ADD CONSTRAINT mc_resource_requests_machine_owner_fk
    FOREIGN KEY (requester_machine_id, owner_user_id) REFERENCES mc_machines(machine_id, owner_user_id) ON DELETE CASCADE NOT VALID;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE mc_provider_capabilities ADD CONSTRAINT mc_provider_capabilities_machine_owner_fk
    FOREIGN KEY (provider_machine_id, owner_user_id) REFERENCES mc_machines(machine_id, owner_user_id) ON DELETE CASCADE NOT VALID;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE mc_resource_requests ADD CONSTRAINT mc_resource_requests_capability_provider_fk
    FOREIGN KEY (capability_id, provider_machine_id) REFERENCES mc_provider_capabilities(id, provider_machine_id) ON DELETE RESTRICT NOT VALID;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE mc_settlements ADD CONSTRAINT mc_settlements_request_fk
    FOREIGN KEY (resource_request_id) REFERENCES mc_resource_requests(id) ON DELETE RESTRICT NOT VALID;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE mc_settlements ADD CONSTRAINT mc_settlements_quote_request_fk
    FOREIGN KEY (resource_quote_id, resource_request_id) REFERENCES mc_resource_quotes(id, resource_request_id) ON DELETE RESTRICT NOT VALID;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE mc_settlements ADD CONSTRAINT mc_settlements_request_required
    CHECK (resource_request_id IS NOT NULL AND resource_quote_id IS NOT NULL) NOT VALID;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
...
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE mc_resource_quotes ADD CONSTRAINT mc_resource_quotes_asset_check
    CHECK (asset='SOL') NOT VALID;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
...
-- Phase-2 hardening: all upgrade constraints are validated after the schema
-- has installed them. This turns legacy-row auditing into an executable
-- migration gate instead of leaving production constraints indefinitely
-- marked NOT VALID. PostgreSQL already enforces NOT VALID constraints for new
-- writes; VALIDATE additionally proves every existing row.
